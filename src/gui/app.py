...
class NewsletterGenUI:
    ...
    def generate_newsletter(self, topic, personal_message):
        ...
        return NewsletterCrewCrew().crew().kickoff(inputs=inputs)
    ...
    def sidebar(self):
        with st.sidebar:
            st.title("Matplexer's Newsletter Generator")

            st.write(
                """
                To generate a newsletter, enter a topic and a personal message. \n
                Matplexer's team of AI agents will generate a newsletter for you!
                """
            )

            st.text_input("Topic", key="topic", placeholder="Latest news on AI")

            st.text_area(
                "Your personal message (to include at the top of the newsletter)",
                key="personal_message",
                placeholder="Hey there!, Welcome to the Matplexer's newsletter!"
            )
